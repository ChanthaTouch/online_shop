<template>
  <div class="bg-white rounded-lg overflow-hidden hover:shadow-lg transition-shadow h-full flex flex-col">
    ...
    <div class="p-4">
      ...
      <router-link 
        :to="{ name: 'ProductDetail', params: { slug: product.slug } }"
        class="font-bold text-gray-800 hover:text-pink-500 transition line-clamp-2 text-sm md:text-base"
      >
        {{ product.name }}
      </router-link>
      ...
      <button 
        ...
        class="w-full mt-4 px-4 py-2 bg-pink-500 text-white rounded-lg font-semibold hover:bg-pink-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition text-sm"
      >
